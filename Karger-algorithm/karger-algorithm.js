...
class Graph {
  ...
  conractEdge(choosenEdgeIndex) {
    ...
    let [v1, v2] = this.edges[choosenEdgeIndex];
    let v3 = this.vertices[this.vertices.length - 1] + 1;
  ...
  // delete self loops (a self loop is an edge with the same vertex on both source and destination vertices)
  deleteSelfLoops() {
  ...
  karger() {
    while (this.vertices.length > 2) {
      let randomEdgeI = this.chooseRandomEdge();
...
fs.readFile(`${__dirname}/graph.txt`, "utf8", (err, data) => {
  ...
  for (let i = 0; i < arrs.length; i++) {
    ...
    for (let element of arr) {
      ...
      // because I don't want repeated edges... assuming there is no parallel edges at the original graph
      if (element > i + 1) {
        let edge = [i + 1, element];
  ...
  let minCutMemory = [];
  let num = graph.edges.length;
  for (let i = 0; i < num; i++) {
    // start karger
    let minCut = graph.karger();
